...
for (const record of lock.records) {
  ...
  if (coreRoutes.has(record.route)) {
    ...
    if (record.route === '/') {
      result(record.route, 'homepage recovery sections complete', $('.home-hero,.home-intro,.home-featured,.home-service,.home-process,.home-contact-preview,.home-testimonials,.home-final-cta').length === 8);
      result(record.route, 'homepage source guide links retained', $('.home-featured .recovery-article-card').length === 6 && new Set($('.home-featured .recovery-article-card').toArray().map((node) => $(node).attr('href'))).size === 6, String($('.home-featured .recovery-article-card').length));
      ...
      result(record.route, 'homepage process complete', $('.home-process-grid > li').length === 4, String($('.home-process-grid > li').length));
      result(record.route, 'homepage form safely disabled', $('form[data-disabled-enquiry-form]').length === 1 && $('form[data-disabled-enquiry-form] :input:not(:disabled)').length === 0 && !$('form[data-disabled-enquiry-form]').attr('action'));
      result(record.route, 'homepage testimonials retained', $('.home-testimonial-grid blockquote').length === 3, String($('.home-testimonial-grid blockquote').length));
