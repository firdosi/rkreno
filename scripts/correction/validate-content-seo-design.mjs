...
for (const record of lock.records) {
  ...
  if (coreRoutes.has(record.route)) {
    ...
    result(record.route, 'article family selected', $('[data-article-recovery]').length === 1);
    result(record.route, 'article metadata', $('.pu-meta span').length === 3);
    result(record.route, 'article table of contents', $('.pu-toc a').length === 5);
    result(record.route, 'article structured sections', $('[data-pu-section]').length === 5);
    result(record.route, 'article crack causes', $('.pu-cause-grid article').length === 3);
    result(record.route, 'article process steps', $('.pu-process-list li').length === 5);
    result(record.route, 'article pricing structured', $('.pu-price-table tbody tr').length === 3 && /RM 150 - RM 300/.test(mainText) && /RM 3,500 - RM 6,000\+/.test(mainText));
    result(record.route, 'article WhatsApp CTA', $('.pu-page a[href^="https://wa.me/"]').length === 1);
    result(record.route, 'source image purpose retained', $('.pu-feature img').length === 1);
    result(record.route, 'FAQ absent', $('.pu-page details,.pu-page [data-faq]').length === 0 && !/frequently asked|FAQ/i.test(mainText));
    result(record.route, 'no article template artifacts', !/No Comments|Leave A Comment|author avatar|about the author/i.test(mainText) && $('.pu-page img[src*="gravatar"]').length === 0);
    result(record.route, 'no active content form', $('.pu-page form').length === 0);
    ...
    result(record.route, 'article family selected', $('[data-article-recovery]').length === 1);
    result(record.route, 'article metadata', $('.costguide-meta span').length === 3);
    result(record.route, 'article table of contents', $('.costguide-toc a').length === 11);
    result(record.route, 'article structured sections', $('[data-costguide-section]').length === 15);
    result(record.route, 'article pricing structured', $('.costguide-price-grid article').length === 6 && /RM400/.test(mainText) && /RM15,000/.test(mainText));
    result(record.route, 'article budget bands', $('.costguide-budget-grid article').length === 4);
    result(record.route, 'article process steps', $('.costguide-process li').length === 6);
    result(record.route, 'article FAQ accordions', $('.costguide-faq details').length === 9);
    result(record.route, 'article related services', $('.costguide-related article').length === 6);
    result(record.route, 'article final CTA', $('.costguide-final-cta a').length === 1);
  ...
  } else if (record.route === cleaningGuideManifest.route) {
    const mainText = clean($('main').text());
    result(record.route, 'manual article composition', $('[data-pakej-deep-cleaning-rumah-kl]').length === 1 && $('.article-recovery-page,.article-recovery-toc').length === 0);
    result(record.route, 'article family selected', $('[data-article-recovery]').length === 1);
    result(record.route, 'article table of contents', $('.cleaning-toc a').length === 6);
    result(record.route, 'article structured sections', $('[data-cleaning-section]').length === 10);
    result(record.route, 'article pricing structured', $('.cleaning-price-table tbody tr').length === 4 && /RM250 - RM350/.test(mainText) && /RM0.35 - RM0.50 per sqft/.test(mainText));
    result(record.route, 'article FAQ accordions', $('.cleaning-faq details').length === 5);
    result(record.route, 'article WhatsApp CTAs', $('.cleaning-page a[href^="https://wa.me/"]').length === 3);
    result(record.route, 'source image purposes retained', $('.cleaning-page img').length === 8);
    result(record.route, 'documented source-image limitation', new Set($('.cleaning-page img').toArray().map((node) => $(node).attr('src'))).size === 2);
    result(record.route, 'no article template artifacts', !/No Comments|Leave A Comment|author avatar|about the author/i.test(mainText) && $('.cleaning-page img[src*="gravatar"]').length === 0);
    result(record.route, 'no active content form', $('.cleaning-page form').length === 0);
  } else {
