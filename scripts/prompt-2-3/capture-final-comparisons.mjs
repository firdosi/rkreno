...
const phase = process.env.PROMPT23_PHASE || 'before';
const astroBase = (process.env.PROMPT23_ASTRO_URL || 'https://firdosi.github.io/rkreno').replace(/\/$/, '');
const rawRoot = join(root, '.audit-cache', 'prompt-2-3');
const liveDir = join(rawRoot, 'live-wordpress', phase);
const astroDir = join(rawRoot, `astro-${phase}`);
const regressionDir = join(rawRoot, 'full-site-regression');
const visualDir = join(root, 'reports', 'public', 'visuals', 'prompt-2-3');
const groups = {
  'core-services': [
    ['home', '/'], ['services', '/services/'], ['about', '/about-us/'], ['contact', '/contact-us/'],
    ['faq', '/faq/'], ['blog', '/blog/'], ['aircond-service', '/servis-aircond-murah-kl/'],
    ['aircond-install-kl', '/aircond-installation-kl/'], ['aircond-install-selangor', '/upah-pasang-aircond-selangor/'],
    ['building-renovation', '/service/building-renovation/'], ['electrical', '/electrical-services-selangor/'],
    ['house-kl', '/house-renovation-in-kuala-lumpur/'], ['house-selangor', '/house-renovation-in-selangor/'],
    ['subang-renovation', '/home-renovation-contractor-in-subang-jaya/'], ['office-renovation', '/office-renovation-in-kuala-lumpur/'],
    ['waterproofing', '/waterproofing-contractor-kuala-lumpur/'], ['plaster-ceiling', '/plaster-ceiling-contractor-kl/'],
    ['cleaning', '/servis-cuci-rumah-kl/'],
  ],
  articles: [
    ['aircond-install-guide', '/aircond-installation-kl-the-ultimate-2026-guide-rk-reno-solution/'],
    ...
    ['cleaning-guide', '/servis-cuci-rumah-kl-the-ultimate-2026-guide-to-a-spotless-home/'],
  ...
  'archives-utility': [
    ['commercial', '/category/commercial/'], ['hvac-guides', '/category/hvac-guides/'],
    ['maintenance', '/category/maintenance/'], ['renovation', '/category/renovation/'],
    ['servis-pembersihan', '/category/servis-pembersihan/'], ['technical-guides', '/category/technical-guides/'],
    ['interior-finishing', '/tag/interior-finishing/'], ['office-fit-out', '/tag/office-fit-out/'],
    ['waterproofing-tag', '/tag/waterproofing/'], ['thank-you', '/thank-you/'],
    ['not-found', '/__prompt-2-3-page-that-must-not-exist__/'],
  ],
};
const selectedGroups = phase === 'before' ? { 'archives-utility': groups['archives-utility'] } : groups;
const routes = Object.values(selectedGroups).flat();
const viewports = {
...
for (const directory of [liveDir, astroDir, regressionDir, visualDir]) await mkdir(directory, { recursive: true });
...
async function warm(page) {
  await page.addStyleTag({ content: `
    *,*::before,*::after{animation:none!important;transition:none!important}
    .elementor-invisible,.elementor-element,.pxl-swiper-container{opacity:1!important;visibility:visible!important;transform:none!important}` });
...
async function capture(site, base, outputDir, name, route, viewportName, viewport) {
  ...
  const metrics = await page.evaluate(() => {
    ...
    const text = (selector) => [...document.querySelectorAll(selector)].filter(visible)
    ...
    return {
      ...
      faqs: text('main details summary,.rk-faq-item h3').length,
      links: [...document.querySelectorAll('main a[href]')].filter(visible).map((anchor) => ({
        label: clean(anchor.textContent), href: anchor.href,
      })),
  ...
  records.push({ phase, site, name, route, viewportName, status: response?.status(), consoleErrors, ...metrics });
  await page.close();
}
...
async function tile(name, route, viewportName, panelWidth) {
  ...
  const height = labelHeight + Math.max(lm.height, rm.height);
  const label = Buffer.from(`<svg width="${width}" height="${labelHeight}" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="10" y="20" fill="#ff7b00" font-family="Arial" font-size="12" font-weight="700">${xml(name.toUpperCase())}</text>
    <text x="10" y="39" fill="#fff" font-family="Arial" font-size="8">${xml(route)}</text>
    <text x="${panelWidth / 2}" y="59" text-anchor="middle" fill="#fff" font-family="Arial" font-size="9">CURRENT WORDPRESS</text>
    <text x="${panelWidth + gap + panelWidth / 2}" y="59" text-anchor="middle" fill="#fff" font-family="Arial" font-size="9">FINAL ASTRO</text>
  </svg>`);
  const image = await sharp({ create: { width, height, channels: 3, background: '#e8ebef' } }).composite([
    { input: label, left: 0, top: 0 }, { input: left, left: 0, top: labelHeight },
    { input: right, left: panelWidth + gap, top: labelHeight },
  ]).png().toBuffer();
  return { image, width, height };
}

async function sheet(groupName, viewportName, output) {
  const panelWidth = viewportName === 'desktop' ? 220 : 145;
  const tiles = [];
  for (const [name, route] of groups[groupName]) tiles.push(await tile(name, route, viewportName, panelWidth));
  const columns = groupName === 'archives-utility' ? 2 : 3;
  const gap = 12;
  ...
  await sharp({ create: { width, height, channels: 3, background: '#dfe3e8' } }).composite(composites)
    .png({ compressionLevel: 9, palette: true, colours: 160, quality: 84 }).toFile(output);
}

if (phase === 'after') {
  for (const groupName of Object.keys(groups)) {
    for (const viewportName of ['desktop', 'mobile']) {
      await sheet(groupName, viewportName, join(visualDir, `${groupName}-${viewportName}-final-side-by-side.png`));
    }
  }
}
...
const failures = records.filter((record) => {
  const expectedStatus = record.name === 'not-found' ? 404 : 200;
  return record.status !== expectedStatus || record.scrollWidth > record.clientWidth
...
console.log(JSON.stringify({ phase, captures: records.length, failures: failures.map(
  ({ site, route, viewportName, status }) => ({ site, route, viewportName, status }),
) }, null, 2));
